...
########################
#load all comp season databases
...
#load FAC season database
fileName <- paste0("~/Documents/Dropbox files/AI LFC data/Data Sets/WhoScored/Databases/", FACDataBaseName, ".xlsx")
FACSeasonData <- read_xlsx(fileName)

#add in team group to data base
FACSeasonData <- FACSeasonData %>%
  mutate(Team_Group = ifelse(TeamName == "Liverpool", "Liverpool", "All Oppo"))

#load LC season database
fileName <- paste0("~/Documents/Dropbox files/AI LFC data/Data Sets/WhoScored/Databases/", LCDataBaseName, ".xlsx")
LCSeasonData <- read_xlsx(fileName)

#add in team group to data base
LCSeasonData <- LCSeasonData %>%
  mutate(Team_Group = ifelse(TeamName == "Liverpool", "Liverpool", "All Oppo"))
...
#set match date
Game_date <- last(na.omit(plottingData$Game_date))

#set Game name
GameName <- paste0(Match_Home," vs ", Match_Away)

#set fixture
Match_Fixture <- paste0(Match_Home, " vs ", Match_Away, " up to - ", Game_date)
...
social_caption <- function(twitter="@Barts78",
  ...
  for (name in names(social)) {
    ...
    html = glue("<span style='font-family:\"Font Awesome 7 Brands\";color:{icon_color};font-size:8px;'>{icon};</span>
                <span style='color:{bg_color};font-size:7px;'>.</span>
...
my_datasource <- paste0('<span style="font-family:\'Font Awesome 7 Free\', Arial, sans-serif;color:#454545;font-size:8px;">&#x1F4BE;</span>',
                        '<span style="color:#FFF9FB;font-size:7px;">...</span>',
...
``` {r}
#| label: metric-plot-function
#| echo: false
#| message: false
#| warning: false

plot_metric_dashboard <- function(metric, metric_label, metric_is_percent = FALSE) {

  roll_n <- 5

  base_dat <- MatchPLayerSeasonData %>%
    filter(
      TeamName == Match_Home,
      player_shirt <= 38,
  ...
  team_avg <- base_dat %>%
    summarise(team_avg = mean(.data[[metric]], na.rm = TRUE)) %>%
    pull(team_avg)
  ...
  date_limits <- base_dat %>%
    summarise(
      min_date = floor_date(min(Game_date, na.rm = TRUE), "month"),
      max_date = ceiling_date(max(Game_date, na.rm = TRUE), "month")
    )
  ...
  for (p in players) {
    ...
    cat("::: {.callout-plain title='", p, "'}\n\n", sep = "")

    player_dat <- base_dat %>%
      filter(playerId == p) %>%
      arrange(Game_date) %>%
      mutate(
        metric_roll = slide_dbl(
          .data[[metric]],
          ~ mean(.x, na.rm = TRUE),
          .before = roll_n - 1
        )
    ...
    p_plot <- ggplot(player_dat, aes(x = Game_date)) +
      geom_line(aes(y = .data[[metric]]),
                linewidth = 0.7,
                alpha = 0.6,
                colour = Color_Home) +
      geom_line(aes(y = metric_roll),
                linewidth = 0.9,
                linetype = "dashed",
                colour = Color_Home) +
      geom_hline(yintercept = team_avg,
                 linewidth = 0.7,
                 colour = "gray35") +
      scale_x_date(
        limits = c(date_limits$min_date, date_limits$max_date),
        breaks = seq(date_limits$min_date, date_limits$max_date, by = "1 month"),
      ...
      labs(
        ...
        subtitle = paste0(
          Match_Fixture,
          ", Solid = Player, Dashed = Rolling Avg (", roll_n,
          "), Grey = Team Avg"
        ),
        x = NULL,
        y = metric_label,
      ...
      theme(
        plot.background = element_rect(colour = "snow", fill = "snow"),
        panel.background = element_rect(colour = "snow", fill = "snow"),
        plot.title = element_text(size = 11, face = "bold", colour = "gray12"),
...
#| message: false
#| warning: false
#| label: player passing profiles setup

PassConnections <- SeasonData 
...
PassConnections <- PassConnections %>%
  filter(type == "Pass",
         TeamName == "Liverpool",
         receiver_team == "Liverpool") %>% 
  select(Passer = playerId, Receiver = receiver, Passer_Team = teamId, Receiver_Team = receiver_team, Successful_Passes, BCs, Key_Passes, Prog_Passes, EPV = EPVvalue, x, y, Endx, Endy) 

PassConnectionsEPV <- PassConnections %>%
  filter(EPV >= 0.020,
         Successful_Passes == 1,
         Passer %in% c("Dominik Szoboszlai", 
                       "Alexis Mac Allister", 
                       "Curtis Jones",
                       "Ryan Gravenberch"))

PassConnectionsKP <- PassConnections %>%
  filter(EPV >= 0.020,
         Successful_Passes == 1,
         Key_Passes == 1,
         Passer %in% c("Dominik Szoboszlai", 
...
PassConnectionsPP <- PassConnections %>%
  filter(EPV >= 0.020,
         Successful_Passes == 1,
         Prog_Passes == 1,
         Passer %in% c("Dominik Szoboszlai", 
...
#| label: SPassing high EPV

EPV_PassesFT <- PassConnectionsEPV %>%
  ggplot(aes(x = x, y = y)) +
  ...
  #geometrics
  geom_segment(
    aes(x = x, y = y, xend = Endx, yend = Endy, color = EPV),
  ...
  facet_wrap(~Passer , nrow = 2) +
  
  #formatting
  labs(title = paste("Midfielders Successful Passes with threat higher than 0.2 EPV"), 
       subtitle = paste0("Increased Red = Higher Threat (EPV)"),
       caption =  My_caption) +
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        ...
        strip.text = element_text(size = 5, colour = Color_Home_Text, face = "bold"),
...
```{r}
#| title: Progressive Pases
#| label: P Passing high EPV

EPV_PassesFT <- PassConnectionsPP %>%
  ggplot(aes(x = x, y = y)) +
  annotate_pitch(dimensions = pitch_opta, fill = "#E5E5E5", colour = "#95A5A6") +
  theme_pitch() +
  annotate("segment", y = 21.1, yend = 21.1, x = 0, xend = 100, linewidth = 0.5, colour = "#95A5A6", linetype = "longdash") +
  ...
  #formatting
  labs(title = paste("Midfielders Successful Progressive Passes with threat higher than 0.2 EPV"), 
       subtitle = paste0("Increased Red = Higher Threat (EPV)"),
       caption =  My_caption) +
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        plot.caption = element_markdown(),
...
```{r}
#| title: Key Passes
#| label: KP Passing high EPV

EPV_PassesFT <- PassConnectionsKP %>%
  ggplot(aes(x = x, y = y)) +
  annotate_pitch(dimensions = pitch_opta, fill = "#E5E5E5", colour = "#95A5A6") +
  theme_pitch() +
  annotate("segment", y = 21.1, yend = 21.1, x = 0, xend = 100, linewidth = 0.5, colour = "#95A5A6", linetype = "longdash") +
  ...
  #formatting
  labs(title = paste("Midfielders Successful Key Passes with threat higher than 0.2 EPV"), 
       subtitle = paste0("Increased Red = Higher Threat (EPV)"),
       caption =  My_caption) +
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        plot.caption = element_markdown(),
...
### Season per game Profiles {.tabset}
...
metric <- "Pass_Success"
metric_label <- "Passing Success"
metric_is_percent <- TRUE
...
# Open grid container (NO columns)
cat("::: {.player-grid}\n\n")

for (p in players) {

  # Open callout (do NOT close it yet)
  cat("::: {.callout-plain title='", p, "'}\n\n", sep = "")
  ...
  PassingPLinePlot <- ggplot(player_dat, aes(x = Game_date)) +
    geom_line(aes(y = .data[[metric]]),
              linewidth = 0.7,
              alpha = 0.6,
              colour = Color_Home) +
    ...
    labs(
      ...
      subtitle = paste0(
        Match_Fixture,
        ", Solid Line = Player,\nDashed Line = Rolling Avg (", roll_n, " games), Grey Line = Team Avg"
      ),
      x = NULL,
      y = metric_label,
      caption = My_caption
...
metric <- "Successful_Passes"
metric_label <- "Successful Passes"
...
metric <- "receiving_Suc"
metric_label <- "Receiving Success"
metric_is_percent <- TRUE
...
plot_metric_dashboard("Passes_Recieved", "Passes Received", FALSE)
...
plot_metric_dashboard("Team_EPV_P", "Attacking Threat", FALSE)
...
plot_metric_dashboard("EPV_Won", "Defensive Threat", TRUE)
...
plot_metric_dashboard("xG", "Expected Goals", FALSE)
...
plot_metric_dashboard("xGOT", "Expected Goals on Target", FALSE)
...
plot_metric_dashboard("Shot Accuracy", "Shot Accuracy", TRUE)
...
plot_metric_dashboard("Shot Quality", "Shot Quality", TRUE)
...
plot_metric_dashboard("Aerial_Success", "Aerial Success", TRUE)
...
plot_metric_dashboard("Duel_Success", "Duel Success", TRUE)
...
plot_metric_dashboard("Posession_Control", "Possession Control", FALSE)

```

```{r}
#| title: Prog Passes
#| label: Home Pps graph Season P match
#| echo: false
#| message: false
#| warning: false
#| results: asis
...
metric <- "Prog_Passes"
metric_label <- "Progressive Passes"
metric_is_percent <- FALSE
...
#| message: false
#| warning: false
#| label: player passing profiles setup EPL

PassConnections <- EPLSeasonData 
PassConnections <- shift_column(data = PassConnections, columns = "playerId", new_names = "receiver", len = 1, up = TRUE)
PassConnections <- shift_column(data = PassConnections, columns = "TeamName", new_names = "receiver_team", len = 1, up = TRUE)
PassConnections <- shift_column(data = PassConnections, columns = "x", new_names = "Endx", len = 1, up = TRUE)
PassConnections <- shift_column(data = PassConnections, columns = "y", new_names = "Endy", len = 1, up = TRUE)
...
```{r}
#| title: Prog Passes
#| label: Home PPs graph Season P match EPL

PPsLinePlot <- MatchPLayerSeasonData %>%
  filter(
    TeamName == Match_Home,
    player_shirt <= 38,
    !playerId %in% c("Harvey Elliott", "Freddie Woodman", "Giovanni Leoni")
  ...
  ggplot(aes(x = Game_date, y = Prog_Passes, colour = playerId, group = playerId)) +
  geom_line(linewidth = 0.5) +
  #scale_y_continuous(
  #  breaks = seq(0, 1, 0.1),
  #  labels = percent_format(accuracy = 1)
...
```{r}
#| title: Midfielders
#| label: Midfielders Goal Data CL
...
#| message: false
#| warning: false
#| label: player passing profiles setup CL

PassConnections <- CLSeasonData 
PassConnections <- shift_column(data = PassConnections, columns = "playerId", new_names = "receiver", len = 1, up = TRUE)
PassConnections <- shift_column(data = PassConnections, columns = "TeamName", new_names = "receiver_team", len = 1, up = TRUE)
PassConnections <- shift_column(data = PassConnections, columns = "x", new_names = "Endx", len = 1, up = TRUE)
PassConnections <- shift_column(data = PassConnections, columns = "y", new_names = "Endy", len = 1, up = TRUE)
...
#| message: false
#| warning: false
#| label: player Season per match stats tables setup CL

PlayerMatchRole_game <- CLSeasonData %>%
  mutate(
    Out_of_play_time = round(Out_of_play_time / 60, 1),
    EventTime = round(EventTime / 60, 1),
    TeamEPV = if_else(`outcomeType/value` == 1, as.numeric(EPVvalue), 0)
...
```{r}
#| title: Passing Success
#| label: player Passing P Season P match CL
...
```{r}
#| title: Passes Made
#| label: player Passing graph Season P match CL
...
```{r}
#| title: Receiving Success
#| label: player Receiving P graph Season P match CL
...
```{r}
#| title: Receiving Passes
#| label: player receiving graph Season P match CL
...
```{r}
#| title: Attacking Threat
#| label: Home threat graph Season P match CL
...
```{r}
#| title: Defensive Threat
#| label: Home Def threat graph Season P match CL
...
```{r}
#| title: xG
#| label: xG graph Season P match CL
...
```{r}
#| title: xGOT
#| label: xGOT graph Season P match CL
...
```{r}
#| title: Shot Accuracy
#| label: Shot a graph Season P match CL
...
```{r}
#| title: Shot Quality
#| label: shot q graph Season P match CL
...
```{r}
#| title: ADs
#| label: AD graph Season P match CL
...
```{r}
#| title: DAs
#| label: Home DA graph Season P match CL
...
```{r}
#| title: PC
#| label: Home PC graph Season P match CL
...
```{r}
#| title: Prog Passes
#| label: Home PPs graph Season P match CL
...
#| message: false
#| warning: false
#| label: FAC player Season stats tables setup

PlayerMatchRole <- FACSeasonData %>%
  mutate(
    Out_of_play_time = round(Out_of_play_time / 60, 1),
    EventTime = round(EventTime / 60, 1),
    TeamEPV = if_else(`outcomeType/value` == 1, as.numeric(EPVvalue), 0)
...
#| title: Passing stats
#| results: asis
#| label: FAC Home player Passing table Season
...
#| title: Touches and Carries
#| results: asis
#| label: FAC home player Touches table Season
...
```{r}
#| title: Whole Team
#| label: Whole Team Goal Data FAC
#| width: 200
...
```{r}
#| title: Midfielders
#| label: Midfielders Goal Data FAC
...
#| message: false
#| warning: false
#| label: player Season per match stats tables setup FAC

PlayerMatchRole_game <- FACSeasonData %>%
  mutate(
    Out_of_play_time = round(Out_of_play_time / 60, 1),
    EventTime = round(EventTime / 60, 1),
    TeamEPV = if_else(`outcomeType/value` == 1, as.numeric(EPVvalue), 0)
...
```{r}
#| title: Passing Success
#| label: player Passing P Season P match FAC
...
```{r}
#| title: Passes Made
#| label: player Passing graph Season P match FAC
...
```{r}
#| title: Receiving Success
#| label: player Receiving P graph Season P match FAC
...
```{r}
#| title: Receiving Passes
#| label: player receiving graph Season P match FAC
...
```{r}
#| title: Attacking Threat
#| label: Home threat graph Season P match FAC
...
```{r}
#| title: Defensive Threat
#| label: Home Def threat graph Season P match FAC
...
```{r}
#| title: xG
#| label: xG graph Season P match FAC
...
```{r}
#| title: xGOT
#| label: xGOT graph Season P match FAC
...
```{r}
#| title: Shot Accuracy
#| label: Shot a graph Season P match FAC
...
```{r}
#| title: Shot Quality
#| label: shot q graph Season P match FAC
...
```{r}
#| title: ADs
#| label: AD graph Season P match FAC
...
```{r}
#| title: DAs
#| label: Home DA graph Season P match FAC
...
```{r}
#| title: PC
#| label: Home PC graph Season P match FAC
...
```{r}
#| title: Prog Passes
#| label: Home PPs graph Season P match FAC
...
#| title: Passing stats
#| results: asis
#| label: FAC Home player Passing table Season P90
...
#| title: Touches and Carries
#| results: asis
#| label: FAC home player Touches table Season P90
...
#| title: Shooting stats
#| results: asis
#| label: FAC home player Shooting stats table Season P90
...
#| title: Defensive stats
#| results: asis
#| label: FAC home player Defensive stats table Season P90
...
#| message: false
#| warning: false
#| label: LC player Season stats tables setup

PlayerMatchRole <- LCSeasonData %>%
  mutate(
    Out_of_play_time = round(Out_of_play_time / 60, 1),
    EventTime = round(EventTime / 60, 1),
    TeamEPV = if_else(`outcomeType/value` == 1, as.numeric(EPVvalue), 0)
...
#| title: Passing stats
#| results: asis
#| label: LC Home player Passing table Season
...
#| title: Touches and Carries
#| results: asis
#| label: LC home player Touches table Season
...
```{r}
#| title: Midfielders
#| label: Midfielders Goal Data LC
...
#| message: false
#| warning: false
#| label: player Season per match stats tables setup LC

PlayerMatchRole_game <- LCSeasonData %>%
  mutate(
    Out_of_play_time = round(Out_of_play_time / 60, 1),
    EventTime = round(EventTime / 60, 1),
    TeamEPV = if_else(`outcomeType/value` == 1, as.numeric(EPVvalue), 0)
...
```{r}
#| title: Passing Success
#| label: player Passing P Season P match LC
...
```{r}
#| title: Passes Made
#| label: player Passing graph Season P match LC
...
```{r}
#| title: Receiving Success
#| label: player Receiving P graph Season P match LC
...
```{r}
#| title: Receiving Passes
#| label: player receiving graph Season P match LC
...
```{r}
#| title: Attacking Threat
#| label: Home threat graph Season P match LC
...
```{r}
#| title: Defensive Threat
#| label: Home Def threat graph Season P match LC
...
```{r}
#| title: xG
#| label: xG graph Season P match LC
...
```{r}
#| title: xGOT
#| label: xGOT graph Season P match LC
...
```{r}
#| title: Shot Accuracy
#| label: Shot a graph Season P match LC
...
```{r}
#| title: Shot Quality
#| label: shot q graph Season P match LC
...
```{r}
#| title: ADs
#| label: AD graph Season P match LC
...
```{r}
#| title: DAs
#| label: Home DA graph Season P match LC
...
```{r}
#| title: PC
#| label: Home PC graph Season P match LC
...
```{r}
#| title: Prog Passes
#| label: Home PPs graph Season P match LC
...
#| title: Passing stats
#| results: asis
#| label: LC Home player Passing table Season P90
...
#| title: Touches and Carries
#| results: asis
#| label: LC home player Touches table Season P90
...
#| title: Shooting stats
#| results: asis
#| label: LC home player Shooting stats table Season P90
...
#| title: Defensive stats
#| results: asis
#| label: LC home player Defensive stats table Season P90
